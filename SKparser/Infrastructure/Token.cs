...
#region FEATS 
        ...
        public Token(string token, bool hasResult, bool separateFeats)
        {
            string[] splitted = token.Split(new string[] { "\t" }, StringSplitOptions.None);
            Id = splitted[0];
            Form = splitted[1];
        ...
        public Token(string token, bool hasResult, bool separateFeats, bool hasParentCpos, bool hasParentPos, bool hasChildrenPos)
        {
            string[] splitted = token.Split(new string[] { "\t" }, StringSplitOptions.None);
            Id = splitted[0];
            Form = splitted[1];
            ...
            if (hasResult)
            {
                ...
                if (string.IsNullOrEmpty(CorrectHead) && IsCorrectHead)
                    CorrectHead = Head;
                if (string.IsNullOrEmpty(CorrectDeprel) && IsCorrectDeprel)
                    CorrectDeprel = Deprel;
            }
            else
            {
                IsCorrectHead = false;
            ...
            //if (string.IsNullOrEmpty(Cpostag) || string.IsNullOrEmpty(Postag) || string.IsNullOrEmpty(ParentCpos) || string.IsNullOrEmpty(ParentPos) || string.IsNullOrEmpty(GParentCpos) || string.IsNullOrEmpty(GParentPos))
        ...
        public static List<Token> GetTokensFromCoNLLFile(string filepath, bool hasResult, bool separateFeats, bool hasParentCpos, bool hasParentPos, bool hasChildPos)
        {
            ...
            try
            {
                ...
                for (int j = 0; j < lines.Length; j++)
                {
                    if(!string.IsNullOrEmpty(lines[j].Trim()))
                        resultList.Add(new Token(lines[j], hasResult, separateFeats, hasParentCpos, hasParentPos, hasChildPos));
                }
            }
            catch (Exception ex)
            {
        ...
        */
        ...
        public void SaveTokenToFile(Token token, string fileName, bool saveRating, bool hasGPCPosInfo, bool hasGPPosInfo, bool hasChildrenPosInfo)
        {
            try
            {
                ...
                if (hasGPCPosInfo)
                    fileContent += "\t" + token.ParentCpos + "\t" + token.GParentCpos;
                if (hasGPPosInfo)
                    fileContent += "\t" + token.ParentPos + "\t" + token.GParentPos;
                if (hasChildrenPosInfo)
                {
                    fileContent += "\t" + token.ChildrenPos;
                }
                fileContent += "\r\n";
